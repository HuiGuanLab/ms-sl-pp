...
import time
import pickle
from method.model import MS_SL_Net_pp
...
def compute_query2ctx_info(model, eval_dataset, opt, ctx_info):
    ...
    for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing q embedding", total=len(query_eval_loader)):
        ...
        _score_sum = 0.7*_clip_scale_scores + 0.3*_frame_scale_scores
...
def eval_epoch(model, val_video_dataset, val_text_dataset, opt):
    ...
    query_context_scores, global_query_context_scores, score_sum, query_metas = \
        compute_query2ctx_info(model, val_text_dataset, opt, context_info)
...
def setup_model(opt):
    ...
    NAME_TO_MODELS = {'MS_SL_Net_pp':MS_SL_Net_pp}
