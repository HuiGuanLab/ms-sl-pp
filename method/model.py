...
from method.model_components import BertAttention, LinearLayer, BertSelfAttention,\
    TrainablePositionalEncoding,LengthPositionalEncoding
from method.model_components import clip_nce, frame_nce
from method.fast_kmeans import batch_fast_kmedoids, batch_fast_kmedoids_with_split


class MS_SL_Net_pp(nn.Module):
    def __init__(self, config):
        super(MS_SL_Net_pp, self).__init__()
        self.config = config
        ...
        self.frame_pos_embed = TrainablePositionalEncoding(max_position_embeddings=config.max_ctx_l,
                                                          hidden_size=config.hidden_size, dropout=config.input_drop)
        self.length_pos_embed = LengthPositionalEncoding(max_position_embeddings=config.map_size,
                                                          hidden_size=256, dropout=config.input_drop)
        ...
        self.mapping_linear = nn.ModuleList([nn.Linear(config.hidden_size, out_features=config.hidden_size, bias=False)
                                             for i in range(2)])
        self.length_pos = config.length_pos
    ...
    def forward(self, clip_video_feat, frame_video_feat, frame_video_mask, query_feat, query_mask, query_labels):
        ...
        return loss, {"loss_overall": float(loss), 'clip_nce_loss': clip_nce_loss,
                      'clip_trip_loss': clip_trip_loss,'frame_nce_loss': frame_nce_loss, 'frame_trip_loss': frame_trip_loss}
    ...
    def encode_context(self, clip_video_feat, frame_video_feat, video_mask=None):
        ...
        return encoded_frame_feat, vid_proposal_feat_map
    ...
    def encode_feat_map(self, x_feat):
        ...
        if self.cluster:
            if self.length_pos:
                length_emb_proposal_feat_map = self.length_pos_embed(proposal_feat_map)
            else:
                length_emb_proposal_feat_map =  proposal_feat_map
            if self.cluster_num < 64:
                split_size = 64
            elif self.cluster_num < 256:
                split_size = 16
            else:
                split_size = 8
            cluster_assginment, mediods = batch_fast_kmedoids_with_split(length_emb_proposal_feat_map, self.cluster_num, split_size=split_size)
            batch_i = torch.arange(x_feat.shape[0], device=x_feat.device).unsqueeze(1)

            proposal_feat_map = proposal_feat_map[batch_i,mediods]
