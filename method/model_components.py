...
class TrainableLengthPositionalEncoding(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""
    def __init__(self, max_position_embeddings, hidden_size, dropout=0.1):
        super(TrainableLengthPositionalEncoding, self).__init__()
        self.position_embeddings = nn.Embedding(max_position_embeddings, hidden_size)
        self.LayerNorm = nn.LayerNorm(hidden_size)
        self.dropout = nn.Dropout(dropout)
        self.length = self.get_lengths(max_position_embeddings)


    def forward(self, input_feat):
        bsz = input_feat.shape[0]
        position_ids = torch.tensor(copy.deepcopy(self.length),dtype=torch.long, device=input_feat.device)
...
class LengthPositionalEncoding(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""
    def __init__(self, max_position_embeddings, hidden_size, dropout=0.1):
        super(LengthPositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.pe = torch.zeros(max_position_embeddings, hidden_size)
        ...
        self.pe = self.pe.unsqueeze(0)[:,self.length]

    def forward(self, input_feat):
        pos_emb = self.pe.repeat(input_feat.shape[0],1,1).to(input_feat.device)
        input_feat = torch.cat((input_feat, pos_emb), dim=-1)
        return self.dropout(input_feat)
